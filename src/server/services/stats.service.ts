...
export class StatsService {
  static async getStats(db: Database): Promise<AdminStats> {
  ...
  static async getClassUtilisation(
    db: Database,
    limit = 10,
  ): Promise<ClassUtilisationItem[]> {
    const rows = await db
      .select({
        id: classes.id,
        name: classes.name,
  ...
  static async getExpiringMemberships(
    db: Database,
  ): Promise<ExpiringMembershipItem[]> {
    const today = getTodayDateString();
  ...
  static async getCheckinsPerDay(
    db: Database,
  ): Promise<CheckinsPerDayItem[]> {
  ...
  static async getTopTrainers(
    db: Database,
  ): Promise<TopTrainerItem[]> {
    const start = new Date();
    start.setDate(start.getDate() - 14);
    const startStr = start.toISOString().slice(0, 10);
